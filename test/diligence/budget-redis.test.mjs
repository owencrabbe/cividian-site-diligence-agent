...
test("Redis Lua: parallel reservations, caps, settlement, TTL, and outage refusal", { timeout: 20000 }, async () => {
  ...
  try {
    ...
    const outcomes = await Promise.all(Array.from({ length: 12 }, () => reserveRun({ estimateUsd: 0.04, env, connect, now })));
    ...
    assert.equal((await reserveRun({ estimateUsd: 0.05, env, connect, now })).error, "daily_budget_exhausted");
    assert.equal((await reserveRun({ estimateUsd: 0.06, env, connect, now })).error, "per_run_cap_exceeded");
    assert.equal((await reserveRun({ estimateUsd: 0.01, env, now, connect: async () => null })).error, "budget_store_unavailable");
    assert.equal((await reserveRun({ estimateUsd: 0.01, env: { ...env, REDIS_URL: undefined }, now })).error, "budget_store_unavailable");
    ...
    const next = await Promise.all(Array.from({ length: 12 }, () => reserveRun({ estimateUsd: 0.03, env: nextEnv, connect, now: nextDay })));
    ...
    assert.equal((await reserveRun({ estimateUsd: 0.01, env: nextEnv, connect, now: later })).error, "approved_budget_exhausted");
