...
const LIVE_ENV = { DILIGENCE_TAVILY_ENABLED: "1", TAVILY_BUDGET_APPROVAL_REFERENCE: "synthetic-evaluation", TAVILY_APPROVED_CREDITS: "1000", TAVILY_DAILY_CREDITS: "1000", TAVILY_APPROVAL_EXPIRES_AT: "2030-01-01T00:00:00Z", AUTH_SECRET: process.env.AUTH_SECRET, NEBIUS_API_KEY: "scripted-not-real", DILIGENCE_LIVE_INFERENCE: "1", AI_BUDGET_APPROVAL_REFERENCE: "eval", AI_APPROVED_BUDGET_USD: "5", DILIGENCE_DAILY_BUDGET_USD: "1", DILIGENCE_GUEST_INFERENCE: "1" };
...
function modelFor(kind) {
  ...
  if (kind === "scripted_audit_partial" || kind === "scripted_audit_malformed") return { env: LIVE_ENV, complete: async (req) => { const fx = fixture.fixtureReasoning(JSON.parse(req.user)); return answer({ ...fx, supported_findings: [{ statement: "Population for Muncie is 65,194 people, the largest market in east central Indiana.", evidence_ids: ["ev_city_population"] }] }); }, audit: auditor(() => ({ verdict: "partially_supported", unsupported_spans: kind === "scripted_audit_malformed" ? ["the largest market in east central Indiana", "a phrase that is not in the statement"] : ["the largest market in east central Indiana"], reason: "The row gives Muncie's population only; it compares no markets." })) };
...
md.push("## Metrics", "", "- Citation validity: every accepted finding cites only packet ids (enforced by the validator; cases complete_evidence and incorrect_citations).", "- These scripted cases test citation, numeric and audit gates; they do not measure factual accuracy or prove that all unsupported claims are removed. Rejected items are listed per case.", "- Missing information: null values and named reasons (sparse_evidence).", "- City versus site scope: findings carry the narrowest cited scope (city_versus_site_scope).", "- Scenario consistency: arithmetic and readiness criteria (unit tests in test/diligence).", "- Diligence question specificity: each plan item names a verification method and source (question library).", "- Latency and usage: not measured here; live receipts only.", "");
