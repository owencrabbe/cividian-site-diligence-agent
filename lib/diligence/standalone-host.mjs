...
// Public edition identity: temporary guests and isolated managed accounts.
// The private product continues to use its own host seam.
...
import { getAccountSession, hasAccountCookies } from './account-auth.js';
...
export async function getSession(req) {
  if (hasAccountCookies(req)) return getAccountSession(req);
  let token = null;
...
export function sessionVerified(session) { return session?.typ === "diligence-account" && session.verified === true; }
export function sessionSensitiveAllowed(session) { return sessionVerified(session); }
