...
export async function reserveRun({ estimateUsd, env = process.env, now, connect = getRedis }) {
...
export async function settleRun(reservationId, actualUsd, { env = process.env, connect = getRedis } = {}) {
...
export async function budgetSnapshot({ env = process.env, now, connect = getRedis } = {}) {
  const policy = budgetPolicy(env);
  const d = day(now);
  const kind = storeKind(env);
