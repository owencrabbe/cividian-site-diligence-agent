...
import { modelPricing } from "./nebius.js";
...
// Every reservation names the model it pays for. A model whose price was never
// verified from a Nebius source is refused here, whatever the caller estimated.
export async function reserveRun({ model, estimateUsd, env = process.env, now, connect = getBudgetRedis }) {
  const pricing = modelPricing(model);
  if (!pricing || !pricing.verified) return { ok: false, error: "price_unverified", model: typeof model === "string" ? model.slice(0, 128) : null };
...
// Nebius does not expose the credit balance through the inference API, so the
// ledger remembers the last provider signal: when a live call last succeeded
// and when one was last refused for an exhausted or expired credit. Two
// timestamps on the existing total record; no new key, no request content.
const SIGNAL_FIELDS = { ok: "lastLiveOkAt", credit_exhausted: "creditExhaustedAt" };
export async function recordProviderSignal(signal, { env = process.env, now, connect = getBudgetRedis } = {}) {
  const field = SIGNAL_FIELDS[signal];
  if (!field) return false;
  const at = (now || new Date()).toISOString();
  const kind = storeKind(env);
  if (kind === "memory") { memory.total[field] = at; return true; }
  if (kind !== "redis") return false;
  try {
    const db = await connect();
    if (!db) return false;
    await db.hSet(KEY_TOTAL, field, at);
    return true;
  } catch { return false; }
}

// Tavily is billed in its own credits, not dollars, so it never touches the
// Nebius caps. The ledger only counts it, for the status block and receipts.
export async function recordTavilyUsage({ calls = 0, credits = 0 } = {}, { env = process.env, connect = getBudgetRedis } = {}) {
  const c = Math.max(0, Math.floor(Number(calls) || 0)), k = Math.max(0, Math.round((Number(credits) || 0) * 100));
  if (!c && !k) return false;
  const kind = storeKind(env);
  if (kind === "memory") { memory.total.tavilyCalls = (memory.total.tavilyCalls || 0) + c; memory.total.tavilyCredits100 = (memory.total.tavilyCredits100 || 0) + k; return true; }
  if (kind !== "redis") return false;
  try {
    const db = await connect();
    if (!db) return false;
    if (c) await db.hIncrBy(KEY_TOTAL, "tavilyCalls", c);
    if (k) await db.hIncrBy(KEY_TOTAL, "tavilyCredits100", k);
    return true;
  } catch { return false; }
}
...
export async function budgetSnapshot({ env = process.env, now, connect = getBudgetRedis } = {}) {
  ...
  function totalSnapshot(rec) {
    ...
    const stamp = (k) => (rec && typeof rec[k] === "string" && Number.isFinite(Date.parse(rec[k])) ? rec[k] : null);
    return { approvedUsd: policy.approvedUsd, expiresAt: policy.expiresAt, totalReservedUsd: reserved, totalSpentUsd: spent, totalRemainingUsd: rec && policy.approvedUsd != null ? Math.max(0, Math.floor(policy.approvedUsd * MICRO) - Number(rec.reserved || 0) - Number(rec.spent || 0)) / MICRO : null, lastLiveOkAt: stamp("lastLiveOkAt"), creditExhaustedAt: stamp("creditExhaustedAt"), tavilyCalls: rec ? Number(rec.tavilyCalls || 0) : null, tavilyCredits: rec ? Number(rec.tavilyCredits100 || 0) / 100 : null };
  }
  if (kind === "memory") {
    const rec = memDay(d);
