...
const AUDIT_LABEL = { supported: "audit: supported", partially_supported: "audit: partly supported", not_audited: "audit: no verdict", audit_unavailable: "audit unavailable" };

// The statement, escaped, with the auditor's unsupported spans struck through.
function auditedStatement(f) {
  let html = esc(f.statement);
  for (const span of (f.audit && f.audit.unsupportedSpans) || []) html = html.split(esc(span)).join(`<s title="Not supported by the cited rows">${esc(span)}</s>`);
  return html;
}

function auditSummary(a) {
  const c = a.verdictCounts || {};
  return (a.outcome === "audited" ? "audited by " : a.outcome === "audit_unavailable" ? "unavailable (" + String(a.cause || "").replace(/_/g, " ") + "), model " : String(a.outcome || "").replace(/_/g, " ") + ", model ") + a.model + ": supported " + (c.supported || 0) + ", partly supported " + (c.partially_supported || 0) + ", removed " + (c.not_supported || 0) + (c.audit_unavailable ? ", unaudited " + c.audit_unavailable : "") + (a.costEstimate && a.costEstimate.usd != null ? ", $" + a.costEstimate.usd.toFixed(6) + " list-price estimate" : "");
}
...
function evidenceRow(r) {
  ...
  const ordinance = r.extraction === "model_output" && /^zoning_/.test(r.key || "") && r.status === "unverified";
  return `<tr><td><code>${esc(r.id)}</code></td><td>${esc(r.fact)}<div class="fine">${esc(r.text)}</div>${ordinance ? `<div class="fine">Read from the ordinance, confirm with the planning office. Text sha256 ${esc(String(r.hash || "").slice(0, 16))}.</div>` : ""}</td><td>${esc(r.scope.level)} <span class="tag">${esc(r.applicability)}</span></td><td><span class="status s-${esc(r.status)}">${esc(r.status)}</span></td><td>${href ? `<a href="${esc(href)}">${esc(r.source.name)}</a>` : esc(r.source.name)}<div class="fine">${esc(r.source.authority)} / ${esc(r.extraction)}</div></td><td>${esc(r.vintage || "n/a")}<div class="fine">retrieved ${esc(date(r.retrievedAt))}</div></td></tr>`;
}
...
@media print{body{background:#fff}.page{padding:0;max-width:none}h2{break-after:avoid}tr{break-inside:avoid}}
  ...
  if (b.reasoning && b.reasoning.output && b.reasoning.output.supported_findings.length) {
    out.push(`<ul>` + b.reasoning.output.supported_findings.map((f) => `<li>${auditedStatement(f)} <span class="cite">[${f.evidence_ids.map(esc).join(", ")}]</span> <span class="tag">${esc(f.scope || "")} scope</span>${f.stale ? '<span class="tag">stale vintage</span>' : ""}${f.audit ? ` <span class="tag">${esc(AUDIT_LABEL[f.audit.verdict] || f.audit.verdict)}</span>` : ""}${f.audit && f.audit.verdict === "partially_supported" && f.audit.reason ? `<div class="fine">Auditor: ${esc(f.audit.reason)}</div>` : ""}</li>`).join("") + `</ul>`);
  }
