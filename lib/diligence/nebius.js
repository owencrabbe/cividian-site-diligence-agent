...
// Other model prices retain their older, explicitly unverified sources.

import { createLogger } from "./host.js";

const log = createLogger("lib/diligence/nebius");

export const NEBIUS_ENDPOINT = "https://api.tokenfactory.us-central1.nebius.com/v1/chat/completions";
...
export const NEMOTRON_MODELS = {
  "nvidia/nemotron-3-super-120b-a12b": { label: "Nemotron 3 Super 120B (A12B)", contextTokens: 262144, inputPer1M: 0.3, outputPer1M: 0.9, pricingVerified: true, pricingAsOf: "2026-09-20", pricingSource: "Nebius Token Factory signed-in model public-endpoint card (Global) and organization Prices table, verified 2026-09-20: https://tokenfactory.nebius.com/models/catalog/text2text/nvidia%2Fnemotron-3-super-120b-a12b and https://tokenfactory.nebius.com/organization/prices. List prices exclude applicable taxes; cost remains an estimate, not an invoice." },
  "nvidia/Nemotron-3_5-Lightning": { label: "Nemotron 3.5 Lightning", contextTokens: 1000000, inputPer1M: 0.06, outputPer1M: 0.24 },
  "nvidia/Nemotron-3-Ultra-550b-a55b": { label: "Nemotron 3 Ultra 550B (A55B)", contextTokens: 1000000, inputPer1M: 1, outputPer1M: 3 },
};
...
export function nebiusStatus(env = process.env) {
  ...
  return { provider: "nebius", endpoint: NEBIUS_ENDPOINT, configured: key && allowed, model, modelAllowed: allowed, modelKnown: known, reasons };
}

// Estimated cost from dated list prices. An estimate, never a billed amount,
// and null when the model is not in the price table.
export function estimateCost(model, inputTokens, outputTokens) {
  const p = NEMOTRON_MODELS[model];
  const it = Number.isFinite(inputTokens) ? inputTokens : null, ot = Number.isFinite(outputTokens) ? outputTokens : null;
  const pricing = p ? { model, inputPer1M: p.inputPer1M, outputPer1M: p.outputPer1M, asOf: p.pricingAsOf || PRICING_AS_OF, source: p.pricingSource || PRICING_SOURCE, verified: p.pricingVerified === true } : null;
  if (!p || it == null || ot == null) return { usd: null, pricing, basis: p ? "usage_missing" : "unknown_model" };
  const usd = (it * p.inputPer1M + ot * p.outputPer1M) / 1e6;
...
export function estimateRequestCost(model, request) {
  ...
  return { ...cost, oneAttemptUsd: cost.usd, usd: cost.usd == null ? null : Math.round(cost.usd * 2e6) / 1e6, basis: "two_attempt_reservation" };
}
...
export async function nebiusComplete(request, opts = {}) {
  ...
  const body = JSON.stringify({
    ...
    // Nemotron 3 Super defaults to a separate thinking phase. This bounded
    // structured task needs its completion allowance for the final JSON.
    // NVIDIA documents this per-request chat-template control; other model
    // families retain their own defaults.
    ...(status.model === DEFAULT_MODEL ? { chat_template_kwargs: { enable_thinking: false } } : {}),
    messages: [
  ...
  try {
    while (attempts < 2) {
      ...
      let response;
      try {
        response = await transport(NEBIUS_ENDPOINT, { method: "POST", signal: controller.signal, redirect: "error", headers: { "content-type": "application/json", authorization: "Bearer " + env.NEBIUS_API_KEY.trim() }, body });
      } catch (e) {
      ...
      if (httpStatus === 401 || httpStatus === 403) return { ...base, attempts, error: "provider_auth", latencyMs: Date.now() - started };
