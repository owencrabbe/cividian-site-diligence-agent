...
// One tolerance rule for every number check: exact, 2% for magnitudes of 100
// or more, 0.05 below that, and a fraction may appear as a percent.
export function numberAllowed(n, allowed) {
  return allowed.some((a) => a === n || (Math.abs(a) >= 100 && Math.abs(a - n) / Math.abs(a) <= 0.02) || (Math.abs(a) < 100 && Math.abs(a - n) <= 0.05) || (a > 0 && a < 1 && Math.abs(a * 100 - n) <= 0.05));
}

function everyNumberAllowed(text, allowed) {
  for (const n of numbersIn(text)) {
    if (Number.isInteger(n) && n >= 0 && n <= 12) continue;
    if (!numberAllowed(n, allowed)) return n;
  }
  return null;
}

// Every number the whole packet makes available, for measuring claims made
// outside the validator (the live evaluation's bare-model mode).
export function packetNumbers(packet) {
  return allowedNumbers(packet, packetIds(packet));
}
...
export function validateReasoning(raw, packet) {
  ...
  if (Array.isArray(raw.supported_findings)) raw.supported_findings.slice(0, 8).forEach((f, i) => {
    ...
    if (available.length !== cited.length) return reject(path, "cites_unverified_evidence");
