...
import { sourceBytes } from "./source-fetch.js";
...
export async function hashDocument(url, j, { transport, deadlineMs = ZONING_LIMITS.pdfMs, maxBytes = ZONING_LIMITS.pdfMaxBytes } = {}) {
  let u;
  ...
  if (!transport) {
    try { const raw = await sourceBytes(url, [u.hostname], maxBytes, { signal: AbortSignal.timeout(deadlineMs) }); return { ok: true, sha256: sha256(raw.bytes), bytes: raw.bytes.length }; }
    catch { return { ok: false, error: "source_fetch_refused" }; }
  }
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), deadlineMs);
  try {
...
export async function readZoning(site, deps = {}) {
  ...
  const gate = liveInferenceStatus(env, nebiusStatus({ ...env, NEBIUS_MODEL: status.readerModel }));
  if (!gate.live || (deps.owner?.kind === "guest" && !gate.guestAllowed)) return stop("live_not_authorized");
  if (deps.signal && deps.signal.aborted) return stop("cancelled");
  // Nothing billable happens unless the reader could run afterwards: a paused
  // or expired credit, an exhausted or unreachable ledger stops the read here,
  // before a Tavily credit is spent. The check before the reader call stays.
  const early = livePause({ env, now: deps.now ? deps.now() : new Date(), budget: await budgetSnapshot({ env, connect: deps.connect }), model: status.readerModel, maxOutputTokens: ZONING_LIMITS.readerMaxTokens });
  if (early) {
    const credit = early.reason === "credit_exhausted" || early.reason === "credit_expired";
    meta.reader = { model: status.readerModel, outcome: credit ? "credit_paused" : early.reason, requestId: null, usage: null, latencyMs: null, costEstimate: null, reservedUsd: null };
    return stop(credit ? "paused" : "budget_refused", early.reason);
  }
  ...
  const search = await (deps.search || tavilySearch)({ query: meta.query, includeDomains: allowedDomains(j), maxResults: ZONING_LIMITS.maxResults }, { env, transport: deps.transport, connect: deps.connect, signal: deps.signal, deadlineMs: Math.min(ZONING_LIMITS.searchMs, left()) });
  ...
  const extract = await (deps.extract || tavilyExtract)({ urls: readable.map((d) => d.url) }, { env, transport: deps.transport, connect: deps.connect, signal: deps.signal, deadlineMs: Math.min(ZONING_LIMITS.extractMs, left()) });
  await pdfHashes;
  ...
  let result;
  try { result = await (deps.complete || nebiusComplete)(request, { env, model, signal: deps.signal, transport: deps.transport, deadlineMs: Math.min(ZONING_LIMITS.readerMs, Math.max(1000, left() - 1000)) }); }
