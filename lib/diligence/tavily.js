...
import { reserveTavily, recordTavilyCall, tavilyPolicy } from "./tavily-budget.js";
...
async function post(url, body, limit, opts) {
  ...
  if (!tavilyPolicy(env).ok) return { ok: false, error: "tavily_not_authorized" };
  const credits = url === TAVILY_SEARCH ? (body.search_depth === "advanced" ? 2 : 1) : Math.ceil(body.urls.length / 5) * (body.extract_depth === "advanced" ? 2 : 1);
  if (!await reserveTavily({ env, connect: opts.connect, credits })) return { ok: false, error: "tavily_budget_refused" };
  if (!await recordTavilyCall(url === TAVILY_SEARCH ? "search" : "extract", { env, connect: opts.connect })) return { ok: false, error: "tavily_meter_unavailable" };
  ...
  if (opts.signal?.aborted) { clearTimeout(timer); return { ok: false, error: "cancelled" }; }
  ...
  } catch { return { ok: false, error: controller.signal.aborted ? "timeout" : "provider_unavailable" }; }
...
export async function tavilySearch({ query, includeDomains = [], maxResults = 5, depth = "basic" }, opts = {}) {
  if (!["basic", "advanced"].includes(depth)) return { ok: false, error: "invalid_depth" };
  const body = { auto_parameters: false, query: String(query || "").slice(0, 400), search_depth: depth, max_results: Math.max(1, Math.min(Number(maxResults) || 5, 10)), include_answer: false, include_raw_content: false, include_images: false, include_usage: true };
