...
import { getJSON, setJSON, createLogger } from "./host.js";
...
export const AGENT_VERSION = "site-diligence-agent/0.1.0";
export const LIMITS = { queryMaxChars: 200, packetMaxBytes: 24000, maxOutputTokens: 1400, providerTimeoutMs: 20000, maxSavedBriefs: 50, guestTtlSeconds: 86400 };
...
export function ownerKey(session) {
  ...
  if (session.guest === true) return { kind: "guest", key: "guest:" + createHash("sha256").update(String(session.jti || session.sub || "guest")).digest("hex").slice(0, 32), ttl: GUEST_TTL };
...
async function persist(owner, brief) {
  const stored = { ...brief, owner: { kind: owner.kind } };
  await setJSON(briefKey(owner, brief.id), stored, owner.ttl || undefined);
  const idx = (await getJSON(indexKey(owner))) || [];
  const next = [brief.id, ...idx.filter((x) => x !== brief.id)].slice(0, LIMITS.maxSavedBriefs);
  await setJSON(indexKey(owner), next, owner.ttl || undefined);
  return stored;
}
