...
import { tavilyPolicy, tavilySnapshot } from "./tavily-budget.js";
...
export async function reasonRun(id, owner, deps = {}, opts = {}) {
  ...
  if (mode === "fixture") {
    ...
    let result;
    try { result = await (deps.complete || nebiusComplete)(request, { env, signal: opts.signal, transport: deps.transport, deadlineMs: LIMITS.providerTimeoutMs }); }
  ...
  // opts.audit === false is for the live evaluation's packet-only mode; the
  // findings still say they were not audited.
  const audited = await auditFindings({ findings: validated.value.supported_findings, packet: built.packet, env, mode: opts.audit === false ? "not_requested" : mode }, { audit: deps.audit, transport: deps.transport, connect: deps.connect, signal: opts.signal, owner });
...
export async function zoningRun(id, owner, deps = {}, opts = {}) {
  ...
  else out = await readZoning(brief.site, { env, ...(deps.zoning || {}), connect: deps.connect, signal: opts.signal, owner });
...
export async function capabilities(env = process.env, deps = {}) {
  ...
  return {
    ...
    zoning: { creditAllowance: await tavilySnapshot({ env, connect: deps.connect }), available: zs.configured && credit.live && tavilyPolicy(env).ok, tavilyKeyPresent: zs.tavilyKeyPresent, readerModel: zs.readerModel, readerPricingVerified: zs.readerPricingVerified, reasons: [...zs.reasons, ...(tavilyPolicy(env).ok ? [] : ["Tavily credit allowance is not authorized"]), ...(credit.live ? [] : ["live AI is not available"])], note: "Ordinance text is read and quoted verbatim; the governing district is confirmed with planning, never assumed." },
    limits: LIMITS,
  };
}
