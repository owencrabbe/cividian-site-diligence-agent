...
// Website, installable app and isolated account/API/MCP server.
// SITE_URL must match the browser origin for cookie-authenticated writes.
...
import * as standaloneAuth from "./standalone-host.mjs";
import { hasAccountCookies } from "./account-auth.js";
function loadHandler(name) {
  if (!handlers.has(name)) handlers.set(name, (async () => {
    if (name === 'diligence') return (await import('../../api/diligence.js')).createDiligenceHandler(standaloneAuth.getSession);
    if (name === 'guest') {
      const handler = (await import('../../api/guest.js')).createGuestHandler(standaloneAuth);
      return (req, res) => hasAccountCookies(req) ? res.status(409).json({ ok: false, error: 'account_session_present', note: 'Sign in again, or sign out before starting a guest session.' }) : handler(req, res);
    }
    if (name === 'account') return (await import('../../api/diligence-account.js')).default;
    if (name === 'public') return (await import('../../api/diligence-public.js')).default;
    return (await import('../../api/diligence-mcp.js')).default;
  })());
  return handlers.get(name);
}
const PAGES = { '/': 'diligence-site.html', '/app': 'diligence.html', '/diligence': 'diligence.html', '/diligence.html': 'diligence.html', '/login': 'diligence-account.html', '/account': 'diligence-account.html', '/developers': 'diligence-developers.html' };
const ASSETS = { '/diligence-shared.css': ['diligence-shared.css', 'text/css'], '/app-icon.svg': ['diligence-app-icon.svg', 'image/svg+xml'], '/manifest.webmanifest': ['diligence-manifest.webmanifest', 'application/manifest+json'], '/sw.js': ['diligence-sw.js', 'text/javascript'] };
async function serveFile(res, name, type) {
  const file = ["public/" + name, name].map((p) => path.join(ROOT, p)).find((p) => existsSync(p));
...
export function createServer() {
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (PAGES[url.pathname]) return await serveFile(res, PAGES[url.pathname], 'text/html');
      if (ASSETS[url.pathname]) return await serveFile(res, ...ASSETS[url.pathname]);
      const name = url.pathname === '/api/diligence' ? 'diligence' : url.pathname === '/api/guest' ? 'guest' : url.pathname === '/api/account' ? 'account' : url.pathname === '/api/mcp' ? 'mcp' : url.pathname.startsWith('/api/v1/') || url.pathname === '/api/openapi.json' ? 'public' : null;
      ...
      if (req.method === "POST" || req.method === "PUT" || req.method === "PATCH") {
        ...
        if (/^application\/json/i.test(type)) { try { req.body = raw.length ? JSON.parse(raw.toString("utf8")) : {}; } catch { return res.status(400).json(name === 'mcp' ? { jsonrpc: '2.0', id: null, error: { code: -32700, message: 'Parse error.' } } : { ok: false, error: 'invalid_json' }); } }
        else req.body = {};
        if (req.body === null || typeof req.body !== "object") return res.status(400).json({ ok: false, error: "invalid_request" });
      }
      phase = "load_handler";
