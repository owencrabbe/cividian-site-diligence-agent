...
import { liveInferenceStatus } from "./budget.js";
import { nebiusStatus } from "./nebius.js";
...
export function applyVerdicts(findings, output) {
  ...
  findings.forEach((f, i) => {
    ...
    const spans = v.verdict === "partially_supported" && Array.isArray(v.unsupported_spans)
      ? [...new Set(v.unsupported_spans.filter((s) => typeof s === "string").map((s) => s.slice(0, AUDIT_LIMITS.spanMaxChars)).filter((s) => s.length >= 2 && statement.includes(s)))].slice(0, AUDIT_LIMITS.spansPerFinding)
      : [];
...
// Structured output is still untrusted. An incomplete or malformed audit
// cannot be reported as a completed audit with missing qualifications.
export function validAuditOutput(output, findings) {
  const exact = (o, keys) => o && typeof o === "object" && !Array.isArray(o) && Object.keys(o).length === keys.length && Object.keys(o).every((k) => keys.includes(k));
  if (!exact(output, ["verdicts"]) || !Array.isArray(output.verdicts) || output.verdicts.length !== findings.length) return false;
  const seen = new Set();
  for (const v of output.verdicts) {
    if (!exact(v, ["finding_id", "verdict", "unsupported_spans", "reason"]) || !VERDICTS.includes(v.verdict) || seen.has(v.finding_id)) return false;
    const index = findings.findIndex((_f, i) => v.finding_id === "f" + (i + 1));
    if (index < 0 || typeof v.reason !== "string" || !v.reason.trim() || v.reason.length > 240 || /[<>\x00-\x1f\x7f]/.test(v.reason)) return false;
    if (!Array.isArray(v.unsupported_spans) || v.unsupported_spans.length > 4 || v.unsupported_spans.some((span) => typeof span !== "string" || span.length < 2 || span.length > 200 || !findings[index].statement.includes(span))) return false;
    if (v.verdict === "partially_supported" && !v.unsupported_spans.length) return false;
    if (v.verdict === "supported" && v.unsupported_spans.length) return false;
...
export async function auditFindings({ findings, packet, env = process.env, mode = "live" }, deps = {}) {
  ...
  if (mode !== "live") return unavailable(findings, model, mode === "fixture" ? "fixture_mode" : mode === "not_requested" ? "not_requested" : "live_unavailable");
  ...
  if (!gate.live || (deps.owner?.kind === "guest" && !gate.guestAllowed)) return unavailable(findings, model, "live_unavailable");
  ...
  if (Buffer.byteLength(request.user) > 30000) return unavailable(findings, model, "audit_packet_over_budget");
  ...
  let result;
  try { result = await (deps.audit || nebiusComplete)(request, { env, model, signal: deps.signal, transport: deps.transport, deadlineMs: AUDIT_LIMITS.deadlineMs }); }
  ...
  if (result.returnedModel !== model || !result.requestId || !Number.isFinite(result.usage?.inputTokens) || !Number.isFinite(result.usage?.outputTokens) || !validAuditOutput(result.output, findings)) {
    const u = unavailable(findings, model, "audit_output_rejected"); u.audit.costEstimate = costEstimate; return u;
  }
