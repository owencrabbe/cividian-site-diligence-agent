...
export const components = (g) => g.type === 'Polygon' ? [g.coordinates] : g.coordinates;
export const points = (g) => components(g).flat(2);
export function finite(v, label, min, max) {
...
function validateTopology(g) {
    ...
    for (let i = 0; i < parts.length; i++)
        for (let j = i + 1; j < parts.length; j++) {
            const a = { type: 'Polygon', coordinates: parts[i] }, b = { type: 'Polygon', coordinates: parts[j] };
...
export function readGeometry(input, space = 'local') {
    ...
    const parts = raw.type === 'Polygon' ? [raw.coordinates] : raw.coordinates;
    ...
    const result = raw.type === 'Polygon' ? { type: 'Polygon', coordinates: clean[0] } : { type: 'MultiPolygon', coordinates: clean };
    if (space === 'local')
        validateTopology(result);
    return result;
}
export function mapGeometry(g, fn) {
...
export function makeFrame(origin) {
...
export function toLocal(p, frame) {
...
export function toGeographic(p, frame) {
...
export function projectBoundary(g, frame) {
...
export function rectangle(width, depth, x = 0, y = 0, rotation = 0) {
    const r = radians(rotation), c = Math.cos(r), s = Math.sin(r);
    const ring = [[-width / 2, -depth / 2], [width / 2, -depth / 2], [width / 2, depth / 2], [-width / 2, depth / 2], [-width / 2, -depth / 2]];
...
export function rectangularSetback(g, distanceM) {
    ...
    const unsupported = { status: 'not_evaluated', geometry: null, note: 'Constant-distance offsets for concave, holed, or multipart boundaries are not enabled. Install and validate the reviewed topology library; no centroid-scaled envelope is substituted.' };
    ...
    const u = [e0[0] / w, e0[1] / w], v = [e1[0] / h, e1[1] / h];
    const p = (x, y) => [a[0] + u[0] * x + v[0] * y, a[1] + u[1] * x + v[1] * y];
    const inset = { type: 'Polygon', coordinates: [[p(distanceM, distanceM), p(w - distanceM, distanceM), p(w - distanceM, h - distanceM), p(distanceM, h - distanceM), p(distanceM, distanceM)]] };
...
export function contains(container, proposal) {
    ...
    for (const [g, other] of [[a, b], [b, a]]) {
        ...
        for (const poly of components(g)) {
            ...
            for (let i = 0; i < ring.length - 1; i++) {
                ...
                const test = [(p[0] + q[0]) / 2 - sign * dy / length * 1e-7, (p[1] + q[1]) / 2 + sign * dx / length * 1e-7];
