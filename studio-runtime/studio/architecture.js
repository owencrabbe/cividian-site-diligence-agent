...
export const ARCHITECTURE_STYLES = ['heritage_brick', 'limestone', 'warm_modern'];
...
export function parseAppearance(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input))
        throw new StudioError('appearance', 'Architectural appearance must be an object.');
    const r = input;
    if (r['schema'] !== 'studio.appearance.v1' || !ARCHITECTURE_STYLES.includes(r['style']))
        throw new StudioError('appearance', 'Choose a supported architectural style.');
    return { schema: 'studio.appearance.v1', style: r['style'],
        ...
        for (const rings of components(plate.geometry))
            for (const [ringIndex, ring] of rings.entries()) {
                ...
                for (let edge = 0; edge < ring.length - 1; edge++) {
                    ...
                    if (length < .01)
                        continue;
                    const direction = [(q[0] - p[0]) / length, (q[1] - p[1]) / length];
                    const sign = (signedArea >= 0 ? 1 : -1) * (ringIndex === 0 ? 1 : -1);
                    const inward = [-direction[1] * sign, direction[0] * sign];
                    ...
                    for (let i = 0; i < count; i++) {
                        ...
                        if (result.length > 6000)
                            throw new StudioError('architecture_budget', 'This building exceeds detailed-view capacity. Reduce its size or bay density; plan and massing remain available.');
                    }
                }
            }
    }
