...
1. **Resolve the site.** Check the address, map and parcel warnings. An address
...
2. **Choose an objective.** Residential infill, mixed-use or adaptive reuse.
   Review the labeled physical defaults and enter only assumptions you know.
   Leave unknown costs and rents blank; zero means a known zero.
3. **Run diligence.** Watch the stages. Missing data remains visible. When
...
4. **Review the brief.** Inspect source rows, compare scenarios, and identify
   what still needs verification. Scenario arithmetic is not a feasibility
